...
// Xử lý AdminController riêng
if ($controllerName === 'AdminController') {
    $resource = isset($url[1]) && $url[1] != '' ? $url[1] : 'product';
    $subaction = isset($url[2]) && $url[2] != '' ? $url[2] : 'list';
    $controller->product($subaction);
} else if (method_exists($controller, $action)) {
    // Kiểm tra và gọi action cho controller thông thường
